...
//    Primary: Supabase Edge Function verify-turnstile
//    Fallback: token-only (graceful degradation)
// ─────────────────────────────────────────────
async function verifyTurnstile(token) {
  ...
  try {
    ...
    if (error) {
      // Edge function unavailable — log & allow with token-only mode
...
if (form) {
  form.addEventListener("submit", async (e) => {
    ...
    if (captchaResult.reason === "edge_unavailable") {
      console.info("[JECS] CAPTCHA token captured; server verification pending.");
    }
